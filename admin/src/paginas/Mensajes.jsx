...
import { listarConversaciones, listarEtiquetas, etiquetasDeChats, vistos, mensajeError } from "../lib/datos.js";
...
export default function Mensajes() {
  ...
  }, [lista, texto, filtro, etiqueta, asignadas]);
  ...
  return (
    <main className="pagina">
      ...
      {etiquetas.length > 0 && (
        <div className="chips">
          {etiquetas.map(e => <Etiqueta key={e.id} etiqueta={e} activa={etiqueta === e.id} onClick={() => setEtiqueta(x => x === e.id ? null : e.id)} />)}
        </div>
      )}
            ...
            <a className="item-link" href={`#/mensajes/${c.telefono}`}>
              ...
              <span className="item-texto">
                ...
                {(asignadas[c.telefono] || []).length > 0 && (
                  <span className="etiquetas-fila">{asignadas[c.telefono].map(id => porId[id] && <Etiqueta key={id} etiqueta={porId[id]} chica />)}</span>
                )}
